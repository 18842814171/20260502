...
// pre-IF stage
assign to_fs_valid  = ~reset && pre_if_ready_go; //修改
assign pre_if_ready_go = ~br_stall; //添加
...
// IF stage
assign fs_ready_go    = ~br_taken;   // 修改：产生阻塞信号
...
always @(posedge clk) begin
    ...
    else if (to_fs_valid && (fs_allowin || br_taken)) begin // 修改：产生阻塞信号
        fs_pc <= nextpc;
    end
end

assign inst_sram_en    = to_fs_valid && (fs_allowin || br_taken) && pre_if_ready_go; // 修改
